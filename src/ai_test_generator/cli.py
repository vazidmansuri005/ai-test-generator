...
from .models import TestSuite, DiagnosisReport, Classification, Confidence
...
@click.option("--model", "-m", default="claude-sonnet-4-20250514", help="Claude model to use")
def diagnose(results_file: str, fmt: str, output: str | None, model: str):
    ...
    with console.status(f"[bold green]Diagnosing {len(failures)} failures with Claude..."):
        diagnoser = FailureDiagnoser(model=model)
